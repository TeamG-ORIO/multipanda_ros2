...
def generate_launch_description():
    ...
    franka_xacro_file = os.path.join(get_package_share_directory('franka_description'), 'robots', 'sim',
                                     'orio_dual_panda_arm_sim.urdf.xacro')
    ...
    moveit_simple_controllers_yaml = load_yaml(
        'franka_moveit_config', 'config/orio_sim_dual_panda_controllers.yaml'
    )
    moveit_controllers = {
    ...
    ros2_controllers_path = os.path.join(
        ...
        'config',
        'orio_sim_dual_panda_ros_controllers.yaml',
    )
    ...
    # Load controllers
    load_controllers = []
    for controller in ['mj_left_arm_controller', 'mj_right_arm_controller', 'joint_state_broadcaster']:
        load_controllers += [
            ExecuteProcess(
    ...
    # #region agent log
    _log_path = '/home/developer/multipanda_ws/src/multipanda_ros2/.cursor/debug-9e431c.log'
    with open(_log_path, 'a') as _f:
        _f.write(json.dumps({"sessionId":"9e431c","runId":"post-fix","hypothesisId":"A","location":"orio_sim_dual_moveit.launch.py:return","message":"generate_launch_description reached return - YAML parse succeeded","timestamp":int(time.time()*1000)}) + '\n')
    # #endregion
